...
AtResultType SimcomAtCommands::DeleteSms(int smsIndex)
{
    SendAt_P(AtCommand::Generic, F("AT+CMGD=%d"), smsIndex);   
    return PopCommandResult();
}
AtResultType SimcomAtCommands::Reset()
{
    SendAt_P(AtCommand::Generic, F("AT+CRESET"));   
    return PopCommandResult();
}
...
AtResultType SimcomAtCommands::EnableLTE(bool enable)
{
